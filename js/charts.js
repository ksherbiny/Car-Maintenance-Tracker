...
const BASE_OPTS = {
  ...
  plugins: {
    ...
    tooltip: {
      callbacks: {
        label: ctx => ` ${Number(ctx.parsed.y ?? ctx.parsed).toLocaleString('ar-EG')} EGP`
      }
    }
  },
  scales: {
  ...
  _instances[canvasId] = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...BASE_OPTS.plugins,
        tooltip: {
          callbacks: { label: ctx => ` ${Number(ctx.parsed.y).toLocaleString('ar-EG')} EGP` }
        }
      }
    }
  });
  ...
  _instances[canvasId] = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: ctx => ` ${ctx.label}: ${Number(ctx.parsed).toLocaleString('ar-EG')} EGP`
          }
        }
      }
    }
